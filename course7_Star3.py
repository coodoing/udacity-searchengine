...
print(min(10,3,4,5,8,9))


##
# Data Structure: ['python':{'a.html':[1,2,3],'b.html':[4,5,6]}]
#
...
def multi_lookup(index, query):
    result = []
    if len(query)==1:
        for entry in index[query[0]]:
            result.append(entry)
        return result
    else:
        temp = []        
        for entry in query:
            temp.append(index[entry])
        #idx=0
        #idx = getminidx(temp)
        ...
        for entry in start:
            ...
            for i in range(1,len(temp)):
                if entry not in temp[i]:
                    return
                else:
                    #print(first)
                    #print(gap)
                    #print(temp[i][entry])
                    if temp[i][entry]==first+gap:
                        gap +=len(query[i])+1                        
                        flag+=1
            #print(flag)
            if flag == len(temp):
...
def add_page_to_index2(index, url, content):
    ...
    for word in words:        
        ...
        #print(pos)
        if index.get(word)!=None and url in index[word]:            
            start+=len(word)
        #value[url].append(pos)            
        add_to_index(index, word, url,pos)
        
def add_to_index2(index, keyword, url,pos):
    if keyword in index and url in index[keyword]:
